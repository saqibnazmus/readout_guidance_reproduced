...
def diffusion_step(
    ...
    def _diffusion_step(latents):
        ...
        # noise_pred_uncond = noise_pred_uncond[0:1, :, :, :]  # Select the first sample in the batch
        # noise_pred_cond = noise_pred_cond[0:1, :, :, :]  # Same for the conditional noise
        noise_pred_uncond, noise_pred_cond = noise_pred.chunk(2)
        print("noise pred uncond:", noise_pred_uncond.shape)
        print("noise pred cond:", noise_pred_cond.shape)
        print("noise pred chunk:", noise_pred.shape)
        ...
        #     guidance_scale_value = guidance_scale_pt[0] 
        #     if isinstance(guidance_scale_value, torch.Tensor):
        #         guidance_scale_pt = guidance_scale_value
        #     else:
        #         guidance_scale_pt = torch.Tensor(guidance_scale_value)
        #     print("guidance scale", guidance_scale_pt)
        ...
        if isinstance(guidance_scale, (ListConfig, list)):
            guidance_scale_pt = torch.tensor(guidance_scale, dtype=torch.float32)
            guidance_scale_pt = guidance_scale_pt[:, None, None, None]  
            if guidance_scale_pt.shape[0] == 1:
                guidance_scale_pt = guidance_scale_pt.expand(-1, 64, -1, -1)
            print("guidance_scale after processing:", guidance_scale_pt.shape, guidance_scale_pt)
            guidance_scale_pt = guidance_scale_pt.to(noise_pred.device).to(noise_pred.dtype)
        else:
            guidance_scale_pt = torch.tensor(guidance_scale, dtype=torch.float32).to(noise_pred.device).to(noise_pred.dtype)
        noise_pred = noise_pred_uncond + guidance_scale_pt * (noise_pred_cond - noise_pred_uncond)
        if controller.edits:
